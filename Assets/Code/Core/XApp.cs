...
public class XApp : MonoBehaviour
{
    private void Start() 
    {
        ...
        XEventManager.GetSingleton().AddEventListener<C4>(XEvent.Event_004_Id, (c3)=>{GLog.Log("4");}, XGroup.Group_Floor002_Id, XGroup.Group_Floor004_Id,XGroup.Group_Floor005_Id);
    
        XEventManager.GetSingleton().TriggerEvent(XEvent.Event_001_Id);
        XEventManager.GetSingleton().TriggerEvent<C3>(XEvent.Event_003_Id, new C3());

        XEventManager.GetSingleton().RemoveEventListener(XEvent.Event_003_Id);
    }
}
